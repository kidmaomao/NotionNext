...
export async function fetchNotionPageBlocks(id, from = undefined, options = {}) {
  ...
  return hydrateMissingPageMentionRecords(pageBlock, cacheKey)
}
...
async function hydrateMissingPageMentionRecords(recordMap, cacheKey) {
  ...
  const missingPageIds = mentionPageIds.filter(id => {
    ...
    return !hasUsableBlockRecord(blockMap[id]) && !hasUsableBlockRecord(blockMap[compactId])
  })
  ...
  if (cacheKey) {
    await setDataToCache(cacheKey, hydratedRecordMap).catch(err => {
