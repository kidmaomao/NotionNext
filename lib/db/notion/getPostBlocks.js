...
export async function fetchNotionPageBlocks(id, from = undefined, options = {}) {
  ...
  const hydratedPageBlock = await hydrateMissingPageMentionRecords(
    pageBlock,
    cacheKey
  )

  if (hasExpiredSignedUrls(hydratedPageBlock)) {
    await refreshSignedUrls(hydratedPageBlock, cacheKey)
  }
  preferStablePdfSignedUrls(hydratedPageBlock)

  return hydratedPageBlock
}

/**
 * Notion can return inline page mention tokens without the referenced page
 * records in the same recordMap. react-notion-x needs those target page blocks
 * to render mention titles and icons, so patch them in from getBlocks.
 */
async function hydrateMissingPageMentionRecords(recordMap, cacheKey) {
  const blockMap = recordMap?.block
  if (!blockMap || typeof blockMap !== 'object') {
    return recordMap
  }

  const mentionPageIds = collectMentionedPageIds(blockMap)
  const missingPageIds = mentionPageIds.filter(id => {
    const compactId = id.replace(/-/g, '')
    return (
      !hasUsableBlockRecord(blockMap[id]) &&
      !hasUsableBlockRecord(blockMap[compactId])
    )
  })
  ...
  console.log('[PageMention] hydrate missing page records:', missingPageIds.length)

  const fetchedBlocks = await fetchInBatches(missingPageIds, 30)
  const fallbackBlocks = await fetchMissingPageBlocksIndividually(
    missingPageIds,
    fetchedBlocks
  )
  const usableFetchedBlocks = filterUsableBlockRecords({
    ...fetchedBlocks,
    ...fallbackBlocks
  })
  const fetchedCount = Object.keys(usableFetchedBlocks).length

  if (fetchedCount === 0) {
    console.warn('[PageMention] no missing page records fetched')
    return recordMap
  }

  const hydratedRecordMap = {
    ...recordMap,
    block: {
      ...blockMap,
      ...usableFetchedBlocks
  ...
  if (cacheKey) {
    await setDataToCache(cacheKey, hydratedRecordMap, null).catch(err => {
      console.warn('[PageMention] failed to update hydrated cache:', err.message)
    })
  }

  console.log('[PageMention] hydrated page records:', fetchedCount)
  return hydratedRecordMap
}

function collectMentionedPageIds(blockMap) {
  const ids = new Set()

  for (const block of Object.values(blockMap)) {
    const value = block?.value?.value || block?.value || block
    collectPageMentionIds(value?.properties, ids)
  }

  return Array.from(ids)
}

function filterUsableBlockRecords(blockMap) {
  const filtered = {}

  for (const [id, block] of Object.entries(blockMap || {})) {
    if (hasUsableBlockRecord(block)) {
      filtered[id] = block
    }
  }
...
async function fetchMissingPageBlocksIndividually(pageIds, existingBlocks) {
  const existingUsableBlocks = filterUsableBlockRecords(existingBlocks)
  const stillMissing = pageIds.filter(id => {
    const compactId = id.replace(/-/g, '')
    return !existingUsableBlocks[id] && !existingUsableBlocks[compactId]
  })
  ...
  for (const id of stillMissing) {
    try {
      ...
      const pageRecordMap = await notionAPI.getPage(id)
      const block = pageRecordMap?.block || pageRecordMap?.recordMap?.block || {}
      const usableBlocks = filterUsableBlockRecords(block)
      Object.assign(fetchedBlocks, usableBlocks)
      console.log(
        '[PageMention] fallback getPage:',
        id,
        Object.keys(usableBlocks).length
      )
    } catch (err) {
      console.warn('[PageMention] fallback getPage failed:', id, err.message)
    }
  }
...
function hasUsableBlockRecord(block) {
  const value = block?.value?.value || block?.value || block
  return Boolean(value?.id && value?.type)
}

function collectPageMentionIds(node, ids) {
  if (!node) return

  if (Array.isArray(node)) {
    if (node[0] === 'p' && typeof node[1] === 'string') {
      const pageId = normalizeMentionPageId(node[1])
      if (pageId) {
        ids.add(pageId)
      }
    }
    ...
    for (const item of node) {
      collectPageMentionIds(item, ids)
    }
    return
  }
  ...
  if (typeof node === 'object') {
    for (const value of Object.values(node)) {
      collectPageMentionIds(value, ids)
    }
  }
}

function normalizeMentionPageId(id) {
